...
type
  ...
  public
    procedure CalcularAn(funcion:String; periodo:Double);
...
begin
  ...
  CalcularAn(EditEcuacion.Text, T );
end;
...
procedure TFormMain.CalcularAn(funcion:String; periodo:Double);
var
  fa, fb, T, R0, a, b, h, sum: Double;
  n, k, m: Integer;
begin
  {         a=T/2
     An = Integral f(t).cos [(2nPi/T)t] dt
           b=-T/2
  }
  T :=  periodo;
  a := T/2; b := -T/2;
  ...
  Matriz.Cells[0,0] := FloatToStr( (0.5)*( a - b ) * (fa+fb) );  //R[0][0]
  n := 1;
  While True do
  begin
       h := b - a / (2**n); // h = hn
       ...
       if abs( StrToFloat(Matriz.Cells[n-1,n]) - StrToFloat(Matriz.Cells[n,n]) ) < 1 Then
       begin
          MemoResultado.Append(Matriz.Cells[n,n]);
          Break;
       end;
       n := n + 1;
  end;
...
begin
     ...
     FormMain.MemoResultado.Append('Calculando f('+FormMain.EditPeriodo.Text+')');
     cantidadVariables := 1;
