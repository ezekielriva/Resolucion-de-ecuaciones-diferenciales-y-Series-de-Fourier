...
{$endif}
   ...
   function Compile(instr : string; vars : string) : string; overload;
...
uses math, FormulaNf, inifiles{$IFDEF FPC}, LResources{$endif};
...
function TArtFormulaN.Compile(instr : string; vars : string) : string;
var c: char;
    varz:StringArray;
begin
 c := decimalseparator;
   ...
   case compiled[i] of
     ...
     chr(F_VAR):
       begin
        ...
        D[cpos].val := varnames[byte(compiled[i])];
     ...
     chr(F_DATA):
       begin
        if cpos > high(D) then setlength(D,high(D)+33);
        D[cpos].val := floattostr((pdouble(@(compiled[i+1])))^);
        D[cpos].diff := '0';
        D[cpos].fv := true;
        inc(cpos);
        inc(i,sizeof(double));
